...
export default function LoginPage() {
  ...
  const form = useForm<LoginFormValues>({ resolver: zodResolver(formSchema), defaultValues: { email: '', password: '' } });
  ...
  const onSubmit: SubmitHandler<LoginFormValues> = async (data) => {
    ...
    try {
      ...
      if (idTokenResult.claims.admin) router.push('/admin');
      else router.push('/');
      router.refresh();
    } catch (error: any) {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4">
            <FormField control={form.control} name="email" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input type="email" placeholder="m@example.com" {...field} disabled={loading}/></FormControl>
                <FormMessage/>
              </FormItem>
            )}/>
